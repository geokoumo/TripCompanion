...
/** Case/accent-insensitive substring match against a city's name. Free text always remains available — this only ever suggests. */
export function suggestCities(query: string, limit = 6): string[] {
  const trimmed = query.trim();
  if (!trimmed) return [];
  const needle = normalize(trimmed);
  return COMMON_CITIES.filter((c) => normalize(c.city).includes(needle))
    .map((c) => c.city)
    .slice(0, limit);
}
...
export function countryForCity(cityName: string): string | undefined {
  return COMMON_CITIES.find((c) => c.city === cityName)?.country;
}
