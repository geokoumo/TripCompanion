-- Adds an optional per-trip cover photo, stored in the existing private
-- trip-files bucket (see src/data/storage/tripFilesBucket.ts's own comment:
-- "a future cover-photo feature belongs here too, not in a second bucket") —
-- this migration only adds the column and threads it through the existing
-- RPCs, no new bucket/policy needed.
--
...
-- every field from every prior migration must be copied forward explicitly.
-- Both function bodies below are the complete 20260913020000 versions with
-- only cover_photo_path added; nothing else has changed.

alter table public.trips add column if not exists cover_photo_path text;
