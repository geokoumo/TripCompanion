-- Adds an optional price/currency to itinerary stops — the domain
-- validation layer (src/domain) checks a price/currency pair for an
-- Activity, but the persisted itinerary_stops table never had anywhere to
-- put one. Both columns are nullable: most stops don't track a cost.
...
-- columns — everything else in both functions is unchanged from
-- 20260830000000_full_trip_rpcs.sql.
