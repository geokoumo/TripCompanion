-- F-12: distinguishes "this stop is assigned to everyone" (the existing,
-- unchanged meaning of an empty itinerary_stop_travelers set) from "this
-- stop was assigned to one specific traveler who has since been removed
-- from the trip" — see src/features/travelers/lib/travelerRemoval.ts and
-- src/features/itinerary/types.ts's assignedToNobody field. Purely
-- additive: the new column defaults to false for every existing row, which
-- preserves their current "empty travelerIds = everyone" meaning exactly.

alter table public.itinerary_stops
  add column assigned_to_nobody boolean not null default false;

-- get_full_trip / upsert_full_trip re-defined to read/write the new column.
--
...
-- both bodies below are the complete 20260916000000_trip_cover_photo.sql
-- versions (description, coverPhotoPath, bookingItems, documents and all)
-- with only assignedToNobody/assigned_to_nobody added; nothing else changed.
